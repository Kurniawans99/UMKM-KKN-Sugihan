...
export default function HeroSection({ stats }: { stats?: HeroStats }) {
  ...
  return (
    <section className="hero-gradient text-white py-10 sm:py-20 lg:py-24 relative shadow-md">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="max-w-3xl">
          {/* Badge */}
          <div className="animate-fade-in mb-4 sm:mb-6">
            <span className="inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1 sm:py-1.5 rounded-full bg-white/20 text-white text-[11px] sm:text-sm font-semibold backdrop-blur-md border border-white/30 shadow-xs max-w-full truncate">
              <span className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-amber-400 animate-pulse shrink-0" />
              <span className="truncate">Desa Sugihan • Kec. Tengaran, Kab. Semarang</span>
            </span>
          </div>

          {/* Title */}
          <h1 className="text-2xl sm:text-4xl lg:text-5xl font-extrabold leading-tight mb-3 sm:mb-5 text-white animate-fade-in-up drop-shadow-xs">
            Direktori UMKM
            <br />
          ...
          {/* Description */}
          <p className="text-white/95 text-xs sm:text-lg font-normal leading-relaxed mb-6 sm:mb-8 max-w-2xl animate-fade-in-up delay-200">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-6 animate-fade-in-up delay-300 pt-2 border-t border-white/20">
            {/* Stat 1: Total UMKM */}
            <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-xl sm:rounded-2xl p-3 sm:p-4 transition-all hover:bg-white/15">
              <div className="flex items-center gap-1.5 sm:gap-2 text-amber-300 mb-1">
                <Building2 className="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0" />
                <span className="text-xl sm:text-3xl font-extrabold text-white">
                  {displayTotalUmkm > 0 ? displayTotalUmkm : "200+"}
                </span>
              </div>
              <div className="text-emerald-100 text-[11px] sm:text-sm font-medium leading-tight">
